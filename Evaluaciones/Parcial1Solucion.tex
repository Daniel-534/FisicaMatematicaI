...
\fancyhead[c]{Parcial 1 Solución}
\fancyhead[r]{\today}
\fancyfoot[c]{\thepage}
...
donde $r$ es un número en términos del radio de Bohr $a_0$ y $\gamma(x, y)$, $\Gamma(x, y)$ son las funciones gamma incompletas. Determine el potencial eléctrico para $r\ll 1$ a orden $\mathcal{O}(r^2)$.
...
\begin{tikzpicture}[font=\small]
  \begin{scope}[xshift=0cm]
    \draw[->] (-3.0,0) -- (3.2,0) node[below right] {$t$};
    \draw[->] (0,-0.2) -- (0,1.6) node[left] {$f(t)$};
    \draw[very thick, blue!75!black] (-3,0) -- (-1,0) -- (-1,1) -- (1,1) -- (1,0) -- (3,0);
    \draw[dashed] (-1,0) node[below] {$-T$} -- (-1,1);
    \draw[dashed] (1,0) node[below] {$T$} -- (1,1);
    \node[left] at (0,1) {$1$};
    \node at (0,-0.95) {Pulso rectangular en tiempo ($\Delta t = 2T$)};
  \end{scope}

  \begin{scope}[xshift=9.0cm]
    \draw[->] (-3.2,0) -- (3.4,0) node[below right] {$\omega$};
    \draw[->] (0,-0.2) -- (0,1.7) node[left] {$|F(\omega)|$};
    \draw[very thick, red!75!black, smooth, domain=-3:3, samples=400] plot (\x,{abs(sin(180*\x)/(3.14159*\x + 0.00001))});
    \draw[dashed] (-1,0) -- (-1,0.07);
    \node[below=2pt] at (-1,0) {$-\pi/T$};
    \draw[dashed] (1,0) -- (1,0.07);
    \node[below=2pt] at (1,0) {$\pi/T$};
    \draw[decorate,decoration={brace,mirror,amplitude=5pt}] (-1,-0.55) -- (1,-0.55)
      node[midway,below=7pt] {$\Delta\omega \approx 2\pi/T$};
    \node at (0,-1.50) {Espectro continuo (envolvente tipo sinc)};
  \end{scope}
\end{tikzpicture}
\shorthandon{<>}
\caption{Visualización tiempo-frecuencia del pulso: la duración temporal finita produce un espectro continuo con lóbulo central y ancho de banda mínimo inversamente proporcional a $T$.}
\end{figure}
