...
$$ P(r \leq R) = \int_0^R r^2 |\psi_{200}(r)|^2 \, dr \int_0^\pi \sin\theta \, d\theta \int_0^{2\pi} d\varphi =\int_0^R 4\pi r^2 |\psi_{200}(r)|^2$$
Recuerde que
...
\textbf{\textit{Solución alternativa usando la función gamma incompleta}}

\textbf{Función Gamma incompleta inferior:}
    $$ \gamma(s, X) = \int_0^X t^{s-1} e^{-t} \, dt $$
Que podemos reescribir, por comodidad, así:

$$ \gamma(n+1, X) = \int_0^X x^n e^{-x} \, dx $$

La integral a resolver es:

$$P(r \leq 2r_{200}^{mp}) =  \frac{1}{8} \int_{0}^{X} (x^4 - 4x^3 + 4x^2)e^{-x} \, dx$$

En términos de la función gamma incompleta nos queda

$$ P(r \leq 2r_{200}^{mp}) = \frac{1}{8} \Big[ \gamma(5, X) - 4\gamma(4, X) + 4\gamma(3, X) \Big] $$
